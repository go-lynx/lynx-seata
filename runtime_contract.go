...
func (t *TxSeataClient) registerRuntimePluginAlias() {
	if t == nil || t.rt == nil {
		return
	}
	if err := t.rt.RegisterSharedResource(sharedPluginResourceName, t); err != nil {
		log.Warnf("failed to register Seata shared plugin alias: %v", err)
	}
}

func (t *TxSeataClient) publishRuntimeContract(ready, healthy bool) {
	if t == nil || t.rt == nil {
		return
	}
	for _, item := range []struct {
		...
		if err := t.rt.RegisterSharedResource(item.name, item.value); err != nil {
			log.Warnf("failed to register Seata shared runtime contract %s: %v", item.name, err)
		}
	}
	if err := t.rt.RegisterPrivateResource(privateReadinessResourceName, ready); err != nil {
		log.Warnf("failed to register Seata private readiness resource: %v", err)
	}
	if err := t.rt.RegisterPrivateResource(privateHealthResourceName, healthy); err != nil {
		log.Warnf("failed to register Seata private health resource: %v", err)
	}
}
